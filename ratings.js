...
async function getRestaurants(lat, lng) {
    let url = `https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=${47.608013},${-122.335167}&type=restaurant&rankby=distance&key=${AIzaSyB8B4OCWrPJ5ngv2nMpLrg_Zx6xRMrkL28}`;
    try {
...
async function getRestaurantDetails(placeId) {
    let url = `https://maps.googleapis.com/maps/api/place/details/json?place_id=${placeId}&fields=name,formatted_address,geometry,rating,review&key=${AIzaSyB8B4OCWrPJ5ngv2nMpLrg_Zx6xRMrkL28}`;
    try {
        let res = await fetch(url);
        return await res.json();
    } catch (error) {
